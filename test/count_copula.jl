...
function create_gcm_poisson(n_groups, dist)
           ...
           for i in 1:n_groups
             ...
             ni = length(y)
             V = [ones(ni, ni)]
             gcs[i] = GaussianCopulaVCObs(y, X, V, dist)
           end
           gcms = GaussianCopulaVCModel(Vector(gcs))
           return gcms
       end
...
initialize_model_parameters!(gcm)
@show gcm.β
# update σ2 from β using the MM algorithm
update_Σ!(gcm)
@show gcm.Σ;
loglikelihood!(gcm, true, false)# ≈ 471.19671943091146*2
# @show gcm.∇β
# @show gcm.∇Σ
#
...
# initialize_model_parameters!(logistic_model)
