...
function runtest()
   ...
   T = Float64
    ...
    ftail = "normal$(nsims)reps_sim$(samplesize)_$(clustersize).csv"
    writedlm("normal/mse_beta_multivariate_" * ftail, βMseResults, ',')
    writedlm("normal/mse_Sigma_multivariate_" * ftail, ΣMseResults, ',')
    writedlm("normal/mse_tau_multivariate_" * ftail, τMseResults, ',')
    writedlm("normal/fittimes_multivariate_" * ftail, fittimes, ',')

    writedlm("normal/beta_tau_sigma_coverage_" * ftail, βτΣcoverage, ',')

    # glmm
    writedlm("normal/mse_beta_GLMM_" * ftail, βMseResults_GLMM, ',')
    writedlm("normal/mse_Sigma_GLMM_" * ftail, ΣMseResults_GLMM, ',')
    writedlm("normal/mse_tau_GLMM_" * ftail, τMseResults_GLMM, ',')
    writedlm("normal/fittimes_GLMM_" * ftail, fittimes_GLMM, ',')
end
runtest()
