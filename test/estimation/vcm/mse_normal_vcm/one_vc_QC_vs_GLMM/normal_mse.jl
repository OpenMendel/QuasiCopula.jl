    ...
    for k in 1:length(ns)
        ...
        for j in 1:nsims
            ...
            # try
                ...
                fittime_GLMM = @elapsed gm1 = fit(MixedModel, form, df, Normal(); nAGQ = 25)
                display(gm1)
                @show gm1.β
...
ftail = "normal_vcm$(nsims)reps_sim.csv"
...
writedlm("normal_vcm/beta_sigma_coverage_" * ftail, βΣcoverage, ',')

# glmm
writedlm("normal_vcm/mse_beta_logistic_GLMM_" * ftail, βMseResults_GLMM, ',')
writedlm("normal_vcm/mse_Sigma_logistic_GLMM_" * ftail, ΣMseResults_GLMM, ',')
writedlm("normal_vcm/fittimes_logistic_GLMM_" * ftail, fittimes_GLMM, ',')
