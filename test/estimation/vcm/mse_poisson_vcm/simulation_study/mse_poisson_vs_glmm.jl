...
function runtest()
        ...
        for k in 1:length(ns)
            ...
            for j in 1:nsims
                ...
                try
                    ...
                    fittime_GLMM = @elapsed gm1 = fit(MixedModel, form, df, Poisson(); nAGQ = 7)
                    ...
                    println("random seed is $(123 + j), rep $j obs per person $ni samplesize $m ")
    ...
    writedlm("poisson/mse_beta_" * ftail, βMseResults, ',')
    writedlm("poisson/mse_Sigma_" * ftail, ΣMseResults, ',')
    writedlm("poisson/fittimes_" * ftail, fittimes, ',')

    writedlm("poisson/beta_sigma_coverage_" * ftail, βΣcoverage, ',')

    # glmm
    writedlm("poisson/mse_beta_GLMM_" * ftail, βMseResults_GLMM, ',')
    writedlm("poisson/mse_Sigma_GLMM_" * ftail, ΣMseResults_GLMM, ',')
    writedlm("poisson/fittimes_GLMM_" * ftail, fittimes_GLMM, ',')
end
runtest()
